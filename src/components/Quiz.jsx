...
//     } else {
//       onComplete(newScore); // use calculated score here
//     }
//   };
...
export const Quiz = ({ questions, onComplete, onBack }) => {
  const [currentQuestion, setCurrentQuestion] = useState(0);
  const [selectedOption, setSelectedOption] = useState(null);
  const scoreRef = useRef(0); // useRef holds the true score synchronously
  ...
  const handleAnswer = () => {
    ...
    if (currentQuestion + 1 < questions.length) {
      ...
      onComplete(scoreRef.current); // this now reflects the true score
    }
  };
